...
try {
  ...
  $stt = $db->prepare('INSERT INTO book(isbn, title, price, publish, published) VALUES(:isbn, :title, :price, :publish, :published)');
  $stt->bindValue('isbn', $POST['isbn']);
  $stt->bindValue('title', $POST['title']);
  $stt->bindValue('price', $POST['price']);
  $stt->bindValue('publish', $POST['publish']);
  $stt->bindValue('published', $POST['published']);
